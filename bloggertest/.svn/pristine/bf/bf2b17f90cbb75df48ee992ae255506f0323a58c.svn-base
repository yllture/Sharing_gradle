...
//					alert(aa);
					...
					var downpublishpaperid = $row.attr('paperid');
					$.magnificPopup.open({
						items: {
						...
						callbacks: {
							change: function() {
								_self.dialog.$confirm.on('click', function(e) {
									e.preventDefault();
									$.ajax({
										url: "paperdownpublish",
										type: "get",
										data: {
											"downpublishpaperid": downpublishpaperid,
										},
										success: function(data) {
											...
											if(data.code == "0000") {
												...
												if(stateflag == '0') {
													...
													$('.alert').html('下架成功，此试卷不再商城售卖。').addClass('alert-success').show().delay(1000).fadeOut();
												}
												_self.rowRemove($row);
											}
										},
