...
//	})
	...
	$('body #paper-part-4').on('click', '.delete-sure-btn', function() {
		...
		$.ajax({
			type: "post",
			url: 'tranbankdeleteshortanswers',
			data: JSON.stringify(obj),
			contentType: "application/json; charset=utf-8",
			dataType: 'text',
			success: function(res) {
...
//							$('.alert').html('移动成功').addClass('alert-success').show().delay(1000).fadeOut();
	...
	$('.btn-down').click(function() {
		...
		var flag = false; //下一个能否移动标志位，如果此刻未被选中则flag=true,在此刻被选中已被移动则为true
		$(this).closest('.part-questions-container').find('.check-wrap').toArray().reverse().each(function(index,item) {
			if($(this).find('input[type="checkbox"]').is(':checked') && i != 1 && flag == true) {
				var item1=$(this).closest('.q-item');
				var item2=$(this).closest('.q-item').prev();
				var q1id = parseInt(item1.attr('data-qid'));
...
//							$('.alert').html('下移成功').addClass('alert-success').show().delay(1000).fadeOut();
	...
	$('body').on('click','.summary-list .btn-remove',function(){
		...
		if(divid == 'singlediv') {
			url = 'tranbanksinglekowndelete';
		} else if(divid == 'multiplydiv') {
			url = 'tranbankmultiplekowndelete';
		} else if(divid == 'judgediv') {
			url = 'tranbankmovejudgekowndelete';
		} else if(divid == 'filldiv') {
			url = 'tranbankfillkowndelete';
		} else if(divid == 'shortanswerdiv') {
			url = 'tranbankshortanswerkowndelete';
		}
		$.ajax({
					type: "post",
					url: url,
...
//							alert($(this).find('.label-skill').eq(0).text());
							if($(this).find('.label-skill').eq(0).text()==knowname){
								$(this).find('.label-skill').eq(0).text('已删除');
								$(this).remove();
							}
						});
						var i=1;
	...
	$('.q-list').on('DOMNodeInserted',function(){
		...
		if(divid == 'singlediv') {
			url = 'tranbanksinglekown';
		} else if(divid == 'multiplydiv') {
			url = 'tranbankmultiplekown';
		} else if(divid == 'judgediv') {
			url = 'tranbankmovejudgekown';
		} else if(divid == 'filldiv') {
			url = 'tranbankfillkown';
		} else if(divid == 'shortanswerdiv') {
			url = 'tranbankshortanswerkown';
		}
		var scoreall=parseInt($('.score-wrap').find('.num').text());
		var scores=0;
		...
		$.ajax({
					...
					success: function(data) {
						...
						part.find('.text-golden-yellow').text(score);
//						part.find('.part-name-wrap span').text(score);
						$('.score-wrap').find('.num').text(scoreall+score);
					},
					error: function(e) {
						$('.alert').attr('class', 'alert');
						$('.alert').html('出错！').addClass('alert-danger').show().delay(1000).fadeOut();
